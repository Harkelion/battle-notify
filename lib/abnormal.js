...
module.exports = function AbnormalManager(mod, debug) {
  ...
  function refreshAbnormal(event) {
    const nowMs = Date.now();
    const abnormal = getAbnormal(event.target, event.id);
    if (!abnormal.added) abnormal.added = BigInt(nowMs);
  ...
  function removeAbnormal(event) {
    // Prevent infinite Abnormal End ?
    if (event.id == previousId) return;
    previousId = event.id;
    // Prevent infinite Abnormal End ?
    const nowMs = Date.now();
    const abnormal = getAbnormal(event.target, event.id);
  ...
  function getEntityGidFromPartySidPid(serverId, playerId) {
    ...
    const isValidPartyMember = mod.game.party.partyMembers.findIndex(
      (element) =>
        element.gameId !== 0 &&
    ...
    if (debug)
      console.log(
        `BN => [Party handler] Event to '${
          mod.game.party.partyMembers.find(
            (elo) => elo.serverId === serverId && elo.playerId === playerId
          )?.name || "mod.game.party is reporting does not exist! "
        }' with ${
          isValidPartyMember !== -1 ? "" : "In"
        }valid @index:${isValidPartyMember}.`
      );
