...
const ceilFractionalBigIntMs = (uts) =>
  uts / 1000n + (uts % 1000n !== 0n ? 1n : 0n);
const msRemainingBigInt = (uts, nowMs) => uts - BigInt(nowMs);
const sRemainingBigInt = (uts, nowMs) =>
  ceilFractionalBigIntMs(msRemainingBigInt(uts, nowMs));
...
module.exports = function Notify(mod, debug) {
  ...
  function* tags(message) {
    yield* (message.match(/\{.*?\}/gi) || []).map((raw) => {
      const tag = raw.replace(RegExp(/[\{\}]/, "g"), "").toLowerCase();
      return [tag, raw];
    });
  }
  function processTags(message) {
    ...
    for (const [tag, raw] of tags(message)) {
      ...
      if (typeof replace === typeof undefined) {
        mod.warn(
          new Error(
            `[battle-notify] warning: unhandled or unknown tag "${raw}\nmessage: ${msg}`
          )
        );
        replace = "";
      }
  ...
  this.abnormal = function (message, entity, info) {
    ...
    if (debug)
      console.log(
        `BN => [ABN notification::PRE] ${message} || ${JSON.stringify(
          entity,
    ...
    if (info) {
      message = message.replace(
        "{duration}",
        typeof info.expires !== "undefined"
          ? durationBigInt(info.expires, nowMs)
          : 0
      );
    ...
    if (debug)
      console.log(
        `BN => [ABN notification::END] ${message} || ${JSON.stringify(
          entity,
          (k, v) => (typeof v === "bigint" ? v.toString() + "n" : v)
        )} || ${JSON.stringify(info, (k, v) =>
          typeof v === "bigint" ? v.toString() + "n" : v
